...
class PatientPage extends UI with InheritedPatient {
  ...
  Widget _buildBasicInfoSection(Modifier<Patient> patient) {
    return Column(
      ...
      children: [
        ...
        if (patient().editing)
          FCheckbox(
            ...
            onChange: (value) => patient(patient()..attended = value),
          )
        else
          FTile(
  ...
  Widget _buildDetailedInfoSection(Modifier<Patient> patient) {
    return Column(
      children: [
        patient().editing
            ? FTextField(
                ...
                maxLines: null,
                onChange: (value) => patient(patient()..complaints = value),
              )
            : FTile(
        ...
        patient().editing
            ? FTextField(
                ...
                maxLines: null,
                onChange: (value) => patient(patient()..examination = value),
              )
            : FTile(
        ...
        patient().editing
            ? FTextField(
                ...
                maxLines: null,
                onChange: (value) => patient(patient()..diagnosis = value),
              )
            : FTile(
  ...
  Widget _buildPicturesSection(Modifier<Patient> patient) {
    return Column(
      children: [
        Padding(
          padding: const EdgeInsets.all(8.0),
  ...
  Widget _buildOtherInfoSection(Modifier<Patient> patient) {
    ...
    return FScaffold(
      ...
      content: SingleChildScrollView(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            _buildBasicInfoSection(patient),
            const SizedBox(height: 20),
            _buildDetailedInfoSection(patient),
            const SizedBox(height: 20),
            FButton(
              onPress: () async {
            ...
            _buildPicturesSection(patient),
            const SizedBox(height: 20),
            _buildOtherInfoSection(patient),
          ],
        ),
      ),
    );
...
mixin PatientTypeBloc {
  final typeRM = RM.inject(() => PatientType());
  CollectionModifier<PatientType> get types => patientTypesRepository;
}

class PatientTypeDialog extends UI with PatientTypeBloc {
  PatientTypeDialog({super.key});
      ...
      body: Column(
        ...
        children: types().map(
          (type) {
            return FButton(
              style: FButtonStyle.primary,
              onPress: type.id == typeRM.state.id
                  ? null
                  : () {
                      typeRM
                        ..state = type
